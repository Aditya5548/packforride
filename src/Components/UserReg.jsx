      ...
      <div className='fixed top-0 left-0 z-5 flex justify-center items-center w-screen h-screen bg-gray-200/90'>
        ...
        <div className='flex flex-col gap-2 w-[90%] md:w-100 border border-gray-400 shadow-lg px-10 pt-2 pb-3 bg-white'>
          ...
          <form className='flex flex-col gap-2 pt-2' onSubmit={logcode}>
            ...
            <input type="number" pattern="[0-9]{10}" minLength={10} maxLength={10} placeholder='Phone No' className='w-full outline-none border border-gray-200 px-3 py-1' name="phoneno" value={phoneno} onChange={phonenofilter} required />
            <input type="email" list="emails" placeholder='Email Id' className='w-full outline-none border border-gray-200 px-3 py-1' name="email" value={email} onChange={(e) => { SetEmail(e.target.value) }} required />
            <datalist id="emails">
